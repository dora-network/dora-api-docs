# Trade
## Properties

| Name | Type | Description | Notes |
|------------ | ------------- | ------------- | -------------|
| **transaction\_id** | **UUID** |  | [default to null] |
| **asset\_0** | **UUID** |  | [default to null] |
| **created\_at** | **Date** |  | [default to null] |
| **order\_book\_id** | **UUID** |  | [default to null] |
| **order\_id** | **UUID** |  | [default to null] |
| **order\_seq** | **Integer** |  | [default to null] |
| **price** | **String** |  | [default to null] |
| **quantity\_0** | **String** |  | [default to null] |
| **user\_id** | **UUID** |  | [default to null] |
| **side** | [**Side**](Side.md) |  | [default to null] |
| **aggressor\_indicator** | **Boolean** | If true, then this order is the aggressor (taker); otherwise it is the maker. | [default to null] |

[[Back to Model list]](../README.md#documentation-for-models) [[Back to API list]](../README.md#documentation-for-api-endpoints) [[Back to README]](../README.md)

